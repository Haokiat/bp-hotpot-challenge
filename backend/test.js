...
import { DEPARTMENTS } from './seed.js';

/* Department names are EVENT DATA and change. Tests refer to the first three
   seeded departments rather than naming them, so a new list does not break
   every assertion. */
const [D1, D2, D3] = DEPARTMENTS;
...
/* The ingredient list is EVENT DATA and changes. Tests therefore pick
   ingredients by position and read their real point values, instead of naming
   an ingredient and hard-coding what it is worth. LOW/MID/HIGH are three
   distinct values taken from whatever list is seeded. */
const ingAt = (s, i) => s.listIngredients()[i];
const LOW = (s) => [...s.listIngredients()].sort((a, b) => a.point_value - b.point_value)[0];
const HIGH = (s) => [...s.listIngredients()].sort((a, b) => b.point_value - a.point_value)[0];
const MID = (s) => {
  const vals = [...new Set(s.listIngredients().map((i) => i.point_value))].sort((a, b) => a - b);
  const target = vals[Math.floor(vals.length / 2)];
  return s.listIngredients().find((i) => i.point_value === target);
};

console.log('\nSeed & data model');
check('seeds the confirmed ingredient list with varying point values', () => {
  const s = fresh();
  const ings = s.listIngredients();
  eq(ings.length, 16, 'ingredient count');
...
check('seeds the department list at score 0', () => {
  const s = fresh();
  eq(s.listDepartments().length, DEPARTMENTS.length, 'department count');
...
check('every ingredient maps to a sprite that exists', async () => {
  const s = fresh();
  const { SPRITE_KEYS } = await import('../frontend/sprites.js');
  const keys = new Set(SPRITE_KEYS);
  const missing = s.listIngredients().filter((i) => !keys.has(i.sprite));
  if (missing.length) throw new Error(`no artwork for: ${missing.map((i) => i.name).join(', ')}`);
});
...
check('adds the correct points and creates a ScoreEvent', () => {
  ...
  eq(department.score, i.point_value, 'score after one toss');
  eq(event.points_awarded, i.point_value, 'points_awarded');
  eq(event.department_id, d.id, 'event department');
  eq(event.ingredient_name, i.name, 'event ingredient name');
});

check('accumulates across multiple participants in one department', () => {
  const s = fresh();
  const d = byName(s, D1);
  const expected = LOW(s).point_value + HIGH(s).point_value + MID(s).point_value;
  s.logScore(d.id, LOW(s).id);
  s.logScore(d.id, HIGH(s).id);
  const { department } = s.logScore(d.id, MID(s).id);
  eq(department.score, expected, 'cumulative score');
});

check('keeps departments independent', () => {
  const s = fresh();
  s.logScore(byName(s, D1).id, LOW(s).id);
  s.logScore(byName(s, D2).id, HIGH(s).id);
  eq(byName(s, D1).score, LOW(s).point_value, 'A');
  eq(byName(s, D2).score, HIGH(s).point_value, 'B');
  eq(byName(s, D3).score, 0, 'C');
});

check('rejects an unknown department or ingredient', () => {
  const s = fresh();
  throwsCode(() => s.logScore('dept_nope', LOW(s).id), 'NOT_FOUND');
  throwsCode(() => s.logScore(byName(s, D1).id, 'ing_nope'), 'NOT_FOUND');
});
...
check('removes one toss by id and subtracts exactly its points', () => {
  const s = fresh();
  const a = byName(s, D1);
  s.logScore(a.id, LOW(s).id);       // 10
  const second = s.logScore(a.id, HIGH(s).id).event;
  s.voidScoreEvent(second.id);
  eq(byName(s, D1).score, LOW(s).point_value, 'score after removal');
});

check('reaches an OLD toss, not just the most recent', () => {
  const s = fresh();
  const a = byName(s, D1);
  const first = s.logScore(a.id, HIGH(s).id).event;
  s.logScore(a.id, LOW(s).id);
  s.logScore(a.id, MID(s).id);
  s.voidScoreEvent(first.id);                       // remove the FIRST, not the last
  eq(byName(s, D1).score, LOW(s).point_value + MID(s).point_value,
     'only the chosen toss was removed');
});

check('a removed toss can be restored with its points', () => {
  const s = fresh();
  const a = byName(s, D1);
  const e = s.logScore(a.id, MID(s).id).event; // 15
  s.voidScoreEvent(e.id);
  eq(byName(s, D1).score, 0, 'removed');
  s.restoreScoreEvent(e.id);
  eq(byName(s, D1).score, MID(s).point_value, 'restored');
});

check('refuses to remove or restore twice', () => {
  const s = fresh();
  const a = byName(s, D1);
  const e = s.logScore(a.id, LOW(s).id).event;
  throwsCode(() => s.restoreScoreEvent(e.id), 'NOT_VOIDED');
  s.voidScoreEvent(e.id);
  throwsCode(() => s.voidScoreEvent(e.id), 'ALREADY_VOIDED');
  throwsCode(() => s.replaceScoreEvent(e.id, MID(s).id), 'ALREADY_VOIDED');
});

check('swaps a mis-tapped ingredient and adjusts by the difference', () => {
  const s = fresh();
  const a = byName(s, D1);
  const e = s.logScore(a.id, HIGH(s).id).event;
  s.replaceScoreEvent(e.id, LOW(s).id);
  eq(byName(s, D1).score, LOW(s).point_value, 'adjusted by the delta');
  const row = s.listScoreEvents({ limit: 1 })[0];
  eq(row.ingredient_name, LOW(s).name, 'log shows the corrected ingredient');
...
check('removes the corrected ingredient from the pot', () => {
  ...
  s.logScore(a.id, LOW(s).id);
  const m = s.logScore(a.id, MID(s).id).event;
  eq(s.boardPayload().pot.map((p) => p.name), [LOW(s).name, MID(s).name], 'pot before');
  s.voidScoreEvent(m.id);
  eq(s.boardPayload().pot.map((p) => p.name), [LOW(s).name], 'pot after');
...
check('adds, renames, and rejects duplicate or empty names', () => {
  ...
  throwsCode(() => s.addDepartment(D1.toLowerCase()), 'NAME_TAKEN'); // case-insensitive
...
check('renaming preserves score and soup base', () => {
  ...
  s.logScore(a.id, HIGH(s).id);
  const renamed = s.renameDepartment(a.id, 'Renamed Dept');
  eq(renamed.score, HIGH(s).point_value, 'score kept');
...
check('blocks deleting a department that has scored', () => {
  ...
  eq(byName(s, D1).score, 10, 'score survived the refused delete');
});
...
check('archives a scored department instead, keeping its history', () => {
  ...
  if (byName(s, D1)) throw new Error('archived dept should leave the active list');
  if (s.leaderboard().some((d) => d.name === D1)) throw new Error('should leave leaderboard');
  if (!s.listDepartments(true).some((d) => d.name === D1)) {
...
check('an archived department can be restored with its points intact', () => {
  ...
  const a = byName(s, D1);
  s.setSoupBase(a.id, 'mala');
  s.logScore(a.id, HIGH(s).id); // 20
  s.removeDepartment(a.id, { archive: true });
  eq(s.listArchived().map((d) => d.name), [D1], 'archived list');

  const back = s.restoreDepartment(a.id);
  eq(back.archived, false, 'archived flag');
  eq(back.score, HIGH(s).point_value, 'score preserved');
  eq(back.soup_base, 'mala', 'soup base preserved');
  if (!byName(s, D1)) throw new Error('not back on the active list');
...
check('restoring re-enables scoring into that department', () => {
  ...
  s.logScore(a.id, LOW(s).id);
  s.removeDepartment(a.id, { archive: true });
  throwsCode(() => s.logScore(a.id, LOW(s).id), 'NOT_FOUND');
  s.restoreDepartment(a.id);
  const { department } = s.logScore(a.id, LOW(s).id);
...
check('restore survives another department taking the name meanwhile', () => {
  ...
  s.logScore(a.id, HIGH(s).id);
  s.removeDepartment(a.id, { archive: true });
  s.addDepartment(D1); // name is free again, someone reuses it
  const back = s.restoreDepartment(a.id);
  eq(back.score, HIGH(s).point_value, 'points still restored');
  eq(back.name, `${D1} (2)`, 'suffixed rather than refused');
  eq(s.listDepartments().filter((d) => d.name.startsWith(D1)).length, 2, 'both exist');
});

check('restoring a department that was never archived is refused', () => {
  const s = fresh();
  throwsCode(() => s.restoreDepartment(byName(s, D1).id), 'NOT_ARCHIVED');
...
check('refuses to score into an archived department', () => {
  ...
  throwsCode(() => s.logScore(d.id, LOW(s).id), 'NOT_FOUND');
});
...
check('rejects an unknown soup base', () => {
  const s = fresh();
  throwsCode(() => s.setSoupBase(byName(s, D1).id, 'chicken'), 'VALIDATION');
});

console.log('\nLeaderboard & ties');
check('sorts by score descending, then name', () => {
  const s = fresh();
  s.logScore(byName(s, D3).id, HIGH(s).id);
  s.logScore(byName(s, D1).id, LOW(s).id);
  const scored = s.leaderboard().filter((d) => d.score > 0).map((d) => d.name);
  eq(scored, [D3, D1], 'highest score first');
  const zeros = s.leaderboard().filter((d) => d.score === 0).map((d) => d.name);
  eq(zeros, [...zeros].sort((a, b) => a.localeCompare(b)), 'ties fall back to name order');
});

check('marks EVERY department tied for the top as a winner', () => {
  const s = fresh();
  const top = HIGH(s).id;
  s.logScore(byName(s, D1).id, top);
  s.logScore(byName(s, D2).id, top);
  s.logScore(byName(s, D3).id, LOW(s).id);
  const board = s.leaderboard();
  eq(board.filter((d) => d.is_leader).map((d) => d.name), [D1, D2], 'winners');
  // Competition ranking: the two tied leaders share rank 1, so rank 2 is skipped.
  eq(board.slice(0, 3).map((d) => d.rank), [1, 1, 3], 'ranking skips after a tie');
});
...
// The API still returns every department. The top-three cut (PRD 4.4, revised)
// is applied by the Display Screen, so the admin and the winner logic keep the
// full field.
check('the API returns every department, not a cut', () => {
  const s = fresh();
  const before = s.leaderboard().length;
  for (let i = 0; i < 15; i++) s.addDepartment(`Dept ${i}`);
  eq(s.leaderboard().length, before + 15, 'every department listed');
});

check('reports total score and toss count', () => {
  const s = fresh();
  s.logScore(byName(s, D1).id, MID(s).id); // 15
  s.logScore(byName(s, D2).id, LOW(s).id); // 10
  const p = s.boardPayload();
  eq(p.total_score, MID(s).point_value + LOW(s).point_value, 'total score');
...
check('point value is snapshotted, so removal survives an ingredient edit', () => {
  ...
  s.voidScoreEvent(e.id);
  eq(byName(s, D1).score, 0, 'used the snapshotted 10, not the new 999');
});
...
check('caps the pot at 40 while the score keeps accumulating', () => {
  ...
  eq(byName(s, D1).score, 600, 'score is not');
});
...
console.log('\nDurability (on-disk: journal + snapshots)');
{
  ...
  check('journals the seed, so a replay starts from the same baseline', () => {
    ...
    eq(types.filter((t) => t === 'seed_ingredient').length,
       disk.listIngredients().length, 'seeded ingredients');
    eq(types.filter((t) => t === 'seed_department').length,
       DEPARTMENTS.length, 'seeded departments');
  });

  check('journals every score with the points and resulting total', () => {
    const a = disk.listDepartments().find((d) => d.name === D1);
    ...
    eq(last.department_name, D1, 'department');
  ...
  check('journals a removal and a correction as their own records', () => {
    ...
    const e = disk.logScore(a.id, ings[1].id).event;
    disk.replaceScoreEvent(e.id, ings[0].id);
    const swap = jlines().pop();
    eq(swap.type, 'score_replace', 'replace type');
    eq([swap.from_points, swap.to_points], [ings[1].point_value, ings[0].point_value],
       'points before and after');
    disk.voidScoreEvent(e.id);
    const gone = jlines().pop();
    eq(gone.type, 'score_void', 'void type');
    eq(gone.points, ings[0].point_value, 'points reversed');
  });
  ...
  check('journal survives losing the database entirely (full replay)', () => {
    ...
    const expected = disk.listDepartments().find((d) => d.name === D1).score;
    ...
    const got = rebuilt.prepare('SELECT score FROM departments WHERE name = ?').get(D1);
